...
- After completing common package installation, install casks and packages specific to MacOS
  #+begin_src sh :eval no
    brew install ledger podman skopeo pinentry-mac starship gnu-sed gnupg coreutils dog pandoc
  #+end_src
  #+begin_src sh :eval no
    brew install --cask discord brave-browser keepassxc ripcord tresorit whatsapp mactex iterm2 signal zulip obsidian transmission vlc loom notion
  #+end_src

...
- After completing common package installation, install casks and packages specific to MacOS
  ...
  #+end_src
  #+begin_src sh :eval no
    brew install --cask discord docker iterm2 keepassxc maccy macfuse mactex racket signal transmission tresorit vlc whatsapp zulip
  #+end_src
...
- Install all the tools I use
  #+begin_src fish :eval no
    cargo install --locked atuin bat bend-lang broot cargo-generate choose difftastic dotslash dprint du-dust exa fd-find git-delta hvm hyperfine jless mergiraf procs ripgrep rust-analyzer sd sqlx-cli tagref taplo-cli tealdeer tokei vivid vl-convert websocat zoxide
  #+end_src

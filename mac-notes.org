...
- Install all the tools I use
  #+begin_src fish :eval no
    cargo install atuin bat bend-lang broot cargo-generate choose dotslash dprint du-dust exa fd-find git-delta hvm hyperfine procs ripgrep rust-analyzer sd sqlx-cli tagref tealdeer tokei vivid vl-convert websocat zoxide
  #+end_src
